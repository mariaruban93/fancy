...
$source       = isset($_POST['source']) ? trim((string)$_POST['source']) : 'purchase';
...
try {
  ...
  if ($source === 'opening') {
    if (!table_exists_purchase_cheques($pdo, 'opening_supplier_payments')) {
      throw new RuntimeException('Opening cheque table not available');
    }
    $stmt = $pdo->prepare("SELECT osp.id, COALESCE(osp.branch_id, s.branch_id, 0) AS branch_id
                            FROM opening_supplier_payments osp
                            LEFT JOIN suppliers s ON osp.supplier_id = s.id
                            WHERE osp.id = :id");
    $stmt->execute([':id'=>$payment_id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) throw new RuntimeException('Opening cheque not found.');
    if (($user['role_name'] ?? '') !== 'admin') {
      $myb = (int)($user['branch_id'] ?? 0);
      if ($myb && $myb !== (int)$row['branch_id']) throw new RuntimeException('Not allowed for this branch.');
    }
    $upd = $pdo->prepare("UPDATE opening_supplier_payments SET deposit_date = :dd, deposit_bank_id = :ba, status = 'cleared' WHERE id = :id");
    $upd->execute([':dd'=>$deposit_date, ':ba'=>$bank_id, ':id'=>$payment_id]);
  } else {
