...
$cash_in_hand = $opening_sum + $cash_sales_to_end + $cash_journal_to_end
              - $cash_returns_to_end - $cash_expenses_to_end - $cash_purchase_to_end
              - $worker_cash_to_end - $cargo_cash_to_end;
...
/* Bank journals (exclude obvious inter-branch text) */
$je_sql = "SELECT COALESCE(SUM(je.amount),0)
           ...
           WHERE je.account_type='bank' AND je.entry_date<=:t".$andBranchOnly."
             AND (
               ...
               OR (
                 LOWER(je.description) NOT LIKE '%transfer%'
                 AND LOWER(je.description) NOT LIKE '%inter%'
                 AND LOWER(je.description) NOT LIKE '%branch%'
               )
             )";
