...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setPaymentMethodOptions(selectEl, allowedValues) {
    ...
    selectEl.innerHTML = '';
    all.filter(opt => allowedValues.includes(opt.value))
       .forEach(opt => {
  ...
  if (cardTotal > 0) {
    const commission = Math.round(cardTotal * CARD_COMMISSION_RATE * 100) / 100;
  ...
  byId('finalizePayment').addEventListener('click', function(){
    ...
    // Walk-in final guard: block Credit & Cheque
    if (isWalkIn()) {
      const invalid = paymentMethods.some(pm => WALKIN_DISALLOWED.includes(pm.method));
      if (invalid) {
