  ...
  <div class="row">
    ...
    <div class="col-lg-7">
      <div class="card-box">
        ...
        <!-- Customer due status row: displays current outstanding balance versus due limit -->
        <div class="row mb-2">
        ...
        <!-- Exchange credit row: shows store credit from exchanges that can be applied to current sale -->
  ...
  <div class="modal-dialog"><div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="mb-2">
        ...
        <input type="text" class="form-control" id="editPriceCost" readonly>
      </div>
      <div class="mb-2">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function openPriceModal(i) {
    ...
    byId('editPriceCost').value = fmt(it.cost_price);
  ...
  function addPaymentMethodRow() {
    ...
    row.innerHTML = `
      ...
      <div class="cheque-details row g-2 mb-2" style="display:none;">
        ...
        <div class="col-3"><input type="date" class="form-control cheque-date"></div>
      </div>`;
  ...
  function updateMultiplePaymentTotals() {
    ...
    const roundedPaid = parseFloat(paid.toFixed(2));
    const roundedTotal = parseFloat(total.toFixed(2));
    byId('multipleTotalPaid').textContent = fmt(roundedPaid);

    const diff = parseFloat((roundedTotal - roundedPaid).toFixed(2));
    ...
    if (cardTotal > 0) {
      const commission = parseFloat((cardTotal * CARD_COMMISSION_RATE).toFixed(2));
      const bankNet = parseFloat((cardTotal - commission).toFixed(2));
  ...
  if (method === 'card' && got > 0) {
    const c = parseFloat((got * CARD_COMMISSION_RATE).toFixed(2));
  ...
  byId('finalizePayment').addEventListener('click', function(){
    ...
    paymentMethods = [];
    if (isMultiplePayment) {
      let valid = true, paid = 0;
      document.querySelectorAll('#paymentMethodsContainer .payment-method-row').forEach(row => {
        const method = row.querySelector('.payment-method').value;
        const amt = parseFloat(row.querySelector('.payment-amount').value)||0;
        if (!method || amt <= 0) valid = false;

        // Gather cheque details if method is cheque
        let cheque_number = null, bank_name = null, bank_branch = null, deposit_date = null;
        if (method === 'cheque') {
        ...
        if (method === 'card') {
          commission_rate = CARD_COMMISSION_RATE;
          commission_amount = parseFloat((amt * CARD_COMMISSION_RATE).toFixed(2));
        ...
        paymentMethods.push({
          method,
          amount: amt,
          cheque_number, bank_name, bank_branch, deposit_date,
          commission_rate, commission_amount, bank_amount
        });
        paid += amt;
      });
      if (!valid) return alert('Complete all payment method rows');
      if (paid < total && !confirm('Total paid is less than Grand Total. Mark as partial?')) return;
    } else {
...
if (method === 'credit') {
  // allow 0 or any non-negative amount; remainder becomes DUE
  ...
  if (method === 'cash' && amt < total && !confirm('Received is less than total. Mark as partial?')) return;
      ...
      // CARD commission breakdown (single)
      let commission_rate = null, commission_amount = null, bank_amount = null;
      if (method === 'card') {
        commission_rate = CARD_COMMISSION_RATE;
        commission_amount = parseFloat((amt * CARD_COMMISSION_RATE).toFixed(2));
  ...
  (function() {
    ...
    if (customerSelect && dueStatusEl) {
      ...
      customerSelect.addEventListener('change', updateDueStatus);
      updateDueStatus();
    }
  })();
...
document.getElementById('rm_finalize').addEventListener('click', function () {
  const fb = document.getElementById('rm_feedback');
  fb.classList.add('d-none'); fb.textContent = '';
  ...
  if (typeof cart !== 'undefined' && Array.isArray(cart)) {
    cart.forEach(function (it) {
      items.push({ product_id: it.product_id, batch_id: it.batch_id, qty: it.quantity, price: it.price });
    });
  } else {
    document.querySelectorAll('#cartItems tr').forEach(function (tr) {
      const pid = parseInt(tr.getAttribute('data-product-id'));
      const bid = parseInt(tr.getAttribute('data-batch-id'));
      const qtyEl = tr.querySelector('input.qty');
      const priceEl = tr.querySelector('span.editable-price');
      if (!isNaN(pid)) {
        const qty = qtyEl ? parseInt(qtyEl.value || '1', 10) : 1;
        const price = priceEl ? parseFloat(priceEl.textContent.replace(/[^\d.-]/g, '')) : 0;
        items.push({ product_id: pid, batch_id: bid || 0, qty: qty, price: price });
      }
    });
  }
  const payload = { pin: pin, return_id: rmState.return_id, new_sale: { items: items, grand_total: rmState.new_total } };
  fetch('api/complete_with_return.php', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
  })
    .then(res => res.json())
    .then(js => {
      if (!js.ok) { fb.textContent = js.message || 'Could not finalize merge.'; fb.classList.remove('d-none'); return; }
      if (js.print_url) { window.open(js.print_url, '_blank'); }
      if (typeof cart !== 'undefined' && Array.isArray(cart)) { cart.length = 0; }
      if (document.getElementById('cartItems')) {
        document.getElementById('cartItems').innerHTML = '<tr><td colspan="7" class="text-center py-3">No items added</td></tr>';
      }
    ...
    .catch(() => { fb.textContent = 'Network/Server error finalizing merge.'; fb.classList.remove('d-none'); });
});
</script>
</body></html>
