...
/*
 * ---------------------------------------------------------
 * Include customer opening balances in the total debtors.
 * Some customers may have an initial outstanding amount
 * recorded in the `opening_balance` column of the customers
 * table.  When payments are received against these balances
 * (via the ajax_receive_opening_payment endpoint), the
 * opening_balance value is decremented accordingly.  To
 * include any remaining opening balances in the debtors
 * figure, sum the opening_balance column (filtered by branch
 * where applicable) and add it to the sales minus payments
 * calculation below.
 */
try {
  $customer_opening_total = 0.0;
  // Ensure the customers table has an opening_balance column
  if (col_exists($pdo, 'customers', 'opening_balance')) {
    $sql_open = "SELECT COALESCE(SUM(opening_balance),0) FROM customers WHERE 1=1";
    $params_open = [];
    // If customers table has branch_id, restrict to current branch for non-admins
    if (col_exists($pdo, 'customers', 'branch_id') && (!$is_admin || $branch_id)) {
      $sql_open .= " AND (branch_id = :b)";
      $params_open[':b'] = $branch_id;
    }
    $stmt_open = $pdo->prepare($sql_open);
    $stmt_open->execute($params_open);
    $customer_opening_total = (float)$stmt_open->fetchColumn();
  }
} catch (Throwable $e) {
  $customer_opening_total = 0.0;
}

// Compute final customer debtors including any outstanding opening balances
$customer_debtors = max($sales_to_end - $sales_returns_to_end - $sale_payments_to_end + $customer_opening_total, 0.0);
...
/*
 * ---------------------------------------------------------
 * Include supplier opening balances in accounts payable.
 * Suppliers may have an initial outstanding balance recorded
 * in the `opening_balance` column of the suppliers table.  When
 * payments are made against these balances (via the
 * ajax_pay_supplier_opening endpoint), the opening_balance
 * value is decreased.  Sum any remaining opening balances and
 * add them to the purchase minus payments calculation below.
 */
try {
  $supplier_opening_total = 0.0;
  if (col_exists($pdo, 'suppliers', 'opening_balance')) {
    // Sum supplier opening balances.  If the suppliers table has a branch_id column
    // and a specific branch is being viewed, restrict to that branch.  This prevents
    // opening balances from other branches from appearing in the financial statement.
    $branchClause = '';
    $params = [];
    if ((!$is_admin || $branch_id) && col_exists($pdo, 'suppliers', 'branch_id')) {
      $branchClause = ' WHERE branch_id = :b';
      $params[':b'] = $branch_id;
    }
    $sql_sup_open = "SELECT COALESCE(SUM(opening_balance),0) FROM suppliers".$branchClause;
    $stmt_sup_open = $pdo->prepare($sql_sup_open);
    $stmt_sup_open->execute($params);
    $supplier_opening_total = (float)$stmt_sup_open->fetchColumn();
  }
} catch (Throwable $e) {
  $supplier_opening_total = 0.0;
}

// Compute final supplier payables including any outstanding opening balances
$supplier_payable = max($purch_invoices_to_end - $supplier_cargo_debits - $pur_returns_to_end - $pur_payments_to_end + $supplier_opening_total, 0.0);
...
// Add outstanding opening balances for cargo providers to cargo payable.  These represent amounts owed
// to providers prior to the reporting period.  Only include if the opening_balance column exists.
$cargo_provider_opening_total = 0.0;
if (col_exists($pdo,'cargo_providers','opening_balance')) {
  try {
    // Cargo providers are global (no branch_id column); sum across all
    $stmt = $pdo->query("SELECT COALESCE(SUM(opening_balance),0) FROM cargo_providers");
    $cargo_provider_opening_total = (float)$stmt->fetchColumn();
  } catch (Throwable $e) {
    $cargo_provider_opening_total = 0.0;
  }
}
$cargo_payable = max($cargo_payable + $cargo_provider_opening_total, 0.0);
...
$opening_cash_balance  = compute_branch_cash_balance($pdo, $start_dt, $branch_id, $is_admin);
$opening_bank_balance  = compute_branch_bank_balance($pdo, $start_dt, $branch_id, $is_admin);
// Recalculate opening equity based on actual opening balances (cash, bank, stock)
// and outstanding customer balances.  Supplier opening balances represent
// liabilities that reduce equity, so subtract the supplier opening total
// from the opening equity calculation.  Customer opening balances are assets
// owed to the business and increase equity.
$opening_equity        = $opening_stock_value + $opening_cash_balance + $opening_bank_balance + $customer_opening_total - $supplier_opening_total;
// Deduct cargo provider opening balances from equity (these are liabilities)
if (isset($cargo_provider_opening_total) && is_numeric($cargo_provider_opening_total)) {
  $opening_equity -= $cargo_provider_opening_total;
}
// Closing equity is opening equity plus net profit earned in the period minus drawings taken in the period.
$closing_equity        = $opening_equity + $net_profit - $drawings;
// Total Liabilities & Equity equals payables plus closing equity
